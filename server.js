...
const AI_LANGUAGE = String(process.env.AI_LANGUAGE || "").trim() || null; // force output/reading language, e.g. "Portuguese"
const AI_MODE = String(process.env.AI_MODE || "").trim().toLowerCase() === "transcribe" ? "transcribe" : "answer";
...
if (startupConfigurationError) {
  ...
  console.log(`PenEcho: http://${HOST}:${listeningPort} (${AI_PROVIDER || "invalid provider"}) · modo=${AI_MODE}${AI_LANGUAGE ? ` · idioma=${AI_LANGUAGE}` : ""}`);
